...
class principal:
    ...
    def asignar_imagen(self, url, contador, paciente):
        ...
        imagenes = os.path.join(url,datosnifti[contador])
    ...
    def rotacion(self,id,dic,x):
        for i in dic.keys():
            if id== i:
                ...
                eleccion=int(input("Selecciona la imagen que desea rotar: "))
                ...
                cv2.imwrite(r"C:\Users\Sofia Rojas\Documents\quiz3_Mateo_Diana\rotaciones\imagenrotada_%s.jpg",x)
                plt.subplot(1, 2, 1)
                plt.imshow(mgr)
